...
class Correlation(nn.Module):
    ...
    def __init__(self, kernel_size=1, pad_size=4, max_displacement=4):
    ...
    def forward(self, in1, in2):
        ...
        offsety, offsetx = torch.meshgrid([torch.arange(0, 2 * self.max_displacement + 1),
                                           torch.arange(0, 2 * self.max_displacement + 1)])
        h, w = in1.shape[2], in1.shape[3]
        output = torch.cat([
            torch.mean(in1 * in2_pad[:, :, dy:dy+h, dx:dx+w], 1, keepdim=True)
...
class PWCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        self.corr    = Correlation(kernel_size=1, pad_size=md, max_displacement=md)
